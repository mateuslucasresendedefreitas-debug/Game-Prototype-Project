...
const TIERS: Dictionary = {
	Tier.BASIC:        [&"circle", &"line_v", &"caret", &"line_h"],
	Tier.INTERMEDIATE: [&"triangle", &"square", &"zigzag"],
	Tier.ADVANCED:     [&"s_curve", &"spiral", &"figure_eight"],
	Tier.AMBIGUOUS:    [&"oval", &"rectangle", &"backwards_three"],
}
...
static func _templates_for(symbol_id: StringName) -> Array:
	match symbol_id:
		...
		&"oval":
			return [_arc(140.0, 70.0, 0.0, TAU, 40), _arc(140.0, 70.0, 0.0, -TAU, 40)]
		&"line_v":
			return [_polyline([Vector2(0, -100), Vector2(0, 100)]),
					_polyline([Vector2(0, 100), Vector2(0, -100)])]
		&"line_h":
			return [_polyline([Vector2(-100, 0), Vector2(100, 0)]),
					_polyline([Vector2(100, 0), Vector2(-100, 0)])]
		&"caret":
		...
		&"spiral":
			return [_spiral(3.0, 110.0, 60)]
		&"figure_eight":
			return [_figure_eight(70.0, 50)]
...
static func _figure_eight(radius: float, steps: int) -> Array:
	var pts: Array = []
	for i in steps + 1:
